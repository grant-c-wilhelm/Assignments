...
//get chckedbox data using a loop

document.skiBrands.addEventListener('submit', (e) => {
    e.preventDefault();
    let checkedBoxes = document.querySelectorAll("input[name=likes]:checked")
    let checkedSkiBrands = [];
    for (i = 0; i < checkedBoxes.length; i++) {
        checkedSkiBrands.push(checkedBoxes[i].value)
    }
    console.log(checkedSkiBrands)
})
